...
FROM selected_solubility_data s
...
results_dict = [
    {col: (list(map(float, row[i].split(','))) if isinstance(row[i], str) and col in ['solubility_g_g','solvent_weight_fraction','temperature'] else row[i])
