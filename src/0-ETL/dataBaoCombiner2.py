...
def main():
    ...
    # Get existing solvents
    cursor.execute("SELECT cas, solvent FROM bao_solvents")
    ...
    for cas, solvent in tqdm(rows, desc="Processing solvents"):
        try:
            cid = get_cid_from_name(cas)
            if cid is None:
                logging.warning(f"No PubChem ID found for CAS {cas} (solvent: {solvent})")
                failed_entries += 1
                continue
                
            properties = get_all_properties(cid)
            if properties is None:
                logging.warning(f"No properties found for CAS {cas} (solvent: {solvent})")
                failed_entries += 1
                continue
            ...
            if existing_row:
                # Update existing row by appending the solvent name
                original_names, molecular_name = existing_row
                new_names = f"{original_names}|{solvent}" if original_names else solvent
                ...
                logging.info(f"  Adding Solvent: {solvent}")
                ...
                # Insert new row
                logging.info(f"Adding new solvent - CAS: {cas}, Name: {solvent}")
                cursor.execute("""
            ...
            logging.error(f"Error processing CAS {cas} (solvent: {solvent}): {str(e)}")
            failed_entries += 1
            continue
